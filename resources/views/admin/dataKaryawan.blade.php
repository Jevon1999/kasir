            ...
            <div class="container mx-auto">
                <div class="bg-white p-6 shadow rounded-lg">
                    ...
                    <div class="overflow-x-auto rounded-lg shadow-sm">
                        <table class="min-w-full bg-white rounded-lg overflow-hidden">
                            ...
                            <tbody class="divide-y divide-gray-100 hover:bg-gray-50 transition-colors">
                                ...
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap text-gray-700">{{ $k->name }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-gray-600">{{ $k->email }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="px-3 py-1.5 text-sm font-medium rounded-full
                                            {{ $k->status == 'active' ? 'bg-green-50 text-green-700' : 'bg-yellow-50 text-yellow-700' }}">
                                            {{ ucfirst($k->status) }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap space-x-3">
                                        <a href="{{ route('karyawan.edit', $k->id) }}" class="text-blue-600 hover:text-blue-800 hover:underline">Edit</a>
                                        <span class="text-gray-300">|</span>
                                        <form action="{{ route('karyawan.destroy', $k->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-red-600 hover:text-red-800 hover:underline" onclick="return confirm('Apakah Anda yakin ingin menghapus karyawan ini?')">Hapus</button>
                                        </form>
                                    </td>
                                </tr>
                                @endforeach
                    ...
                    <div class="mt-6 flex justify-end">
                        <a href="{{ route('karyawan.create') }}" class="bg-blue-500 text-white px-5 py-2.5 rounded-lg hover:bg-blue-600 transition-all duration-200 shadow-md hover:shadow-lg flex items-center space-x-2">
